...
class QTL:
    ...
    def get_allele_info(self, allele_file, num_alleles=None):
        ...
        for i,r in enumerate(self._file_iter(allele_file)):
            ...
            # also get allele values for indexing/reporting purposes
            # allele index might not always match it's value, so we want
            # to save the values, in order, so we can report the index, as opposed
    ...
    def allele_val(self, var, index=False):
        ...
        -------
        val : literal
            str that is the value of the allele var corresponds to, or the index 
            of the allele as determined by the relevant input file (allele_file)
            (if index=True).
        """
        id,prnt,depth = tuple(map(int, self[var]))
        val = self.allele_assigns[str(id)][depth][prnt]
        if index:
            return self.allele_vals[depth].index(val)
        else:
            return val
...
if __name__ == '__main__':
    ...
    qtl = QTL(ped, qtl_file, allele_file, pos_file, num_alleles=1)
    print(qtl.allele_info)
    print(qtl.allele_vals)
