...
network representations of pedigrees) and some relevant terminalogy such as
...
paper "QMSim: A Large Scale Genome Simulator for Live Stock" (Sargolzaei and 
Schenkel).
...
of this project, and the effects each parameters has on networks built from data 
generated by QMSim.
...
with QMSim and and which I modified as necessary. When generating new pedigrees, 
I recommend using these files as templates and modifiying them as necessary.
...
sufficiently diverse QTL. Alghouth the QMSim manual states that legal values 
...
number of generations should be $> 20$. Of course, various parameters interralate,
...
and female founder populations and are primarly used, along with `ls`, `ng`, and `pmp` 
...
For example, a founder population consisting of 10 males and 10 females produced a 
segregation network with 840 variables. A founder population consisting of 2 males and 
18 females produced a segregation network with 1480 variables, albeit with more inbreeding.
...
along with `ng`, `ls`, and `male`/`female` are used to control the size of the 
generated pedigree.

##### Population Output Parameters

Population output paramters allow users to specify the data that QMSim generates.
...
Note: the files generated by the `genotype` are required to build segregation
networks.
...
linearly increasing the number of QTL incoporated into a network results in a 
...
Paramter: `nqa` for more information.
...
segregation networks. This parameter has several potential values. See the QMSim 
...
Note: QMSim places all output files in a directories it creates that are named
using the following convention: if QMSim generates data based on the parametes
in the input file `input.prm', it will place all output files in a directory name
`r\_input`.

#### Data Files

Data files are produced by QMSim when the `data` output tag is specified (see the
Populaton Output Parameter section). These files contain all the information necessary
...
specified (see the Populaton Output Parameter section). These files contain the empircal
distributions of allele assignments at each QTL. These files are used to build the
factor tables for cliques made up of single founder nodes in segregations networks
...
assignments for reported individuals in data files. These files are used to 
...
In addition to contstructing pedigrees from the data produced by QMSim, `Pedigree`
...
Although it is assumed that the reader is relatively familiary with segregation
...
individuals are not representated by single nodes, as they would be in a pedigree 
